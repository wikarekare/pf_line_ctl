...
(1..@num_lines).each do |i|
  ...
  else # Line is inactive
    unless @line_ctl.line[i]['failure_map'].nil? # But has a fail over line defined.
...
(1..@num_lines).each do |i|
  unless @host_line_map.line[i].nil? # LINE Has sites assigned to it (even if the line is inactive)
...
if active_lines.length > 0 # at least one line is up ;)
  ...
  (1..@num_lines).each do |i|
    File.open("#{PF_WRK_DIR}/table_line_#{i}", 'w') do |fd|
      unless @host_line_map.line[i].nil?
        # Line per site, outputing the site ip/maskbits for pf to load into tables
        @host_line_map.line[i].each do |_host, network|
          fd.puts "   #{network}"
        end
      end
    end
    File.open("#{PF_WRK_DIR}/table_line_state_#{i}", 'w') do |fd|
      unless @host_line_map.line[i].nil?
        # Site, ip/maskbits, black for on correct line
        @host_line_map.line[i].each do |host, network|
          fd.puts "#{host}\t#{network}\tblack"
        end
      end
    end
  end
